...
PartyQuestNativeSaveEventAdapterResult
PartyQuestNativeSaveEventAdapter::ApplyTrustedProviderRetirement(
    const void* apPayload,
    size_t aPayloadSize,
    const PartyQuestAsyncSaveRequestIdentity& acReservedIdentity,
...
{
    PartyQuestNativeSaveEventAdapterResult result;
    const auto decoded = DecodeRequestRetired(apPayload, aPayloadSize);
    result.DecodeStatus = decoded.Status;
    if (decoded.Status != PartyQuestNativeSaveEventDecodeStatus::Decoded ||
        !decoded.Event)
    {
    ...
    {
    case PartyQuestAsyncSaveContractStatus::Inactive:
        result.Status = PartyQuestNativeSaveEventAdapterStatus::Retired;
        break;
    case PartyQuestAsyncSaveContractStatus::Pending:
        // The provider proved drain, but the logical request must first be
        // failed or cancelled; Retire intentionally refuses silent discard.
        result.Status = PartyQuestNativeSaveEventAdapterStatus::RetirementBeforeTerminal;
        break;
    case PartyQuestAsyncSaveContractStatus::Stale:
        result.Status = PartyQuestNativeSaveEventAdapterStatus::Stale;
        break;
    default:
        result.Status = PartyQuestNativeSaveEventAdapterStatus::ContractRejected;
        break;
    }
    result.ContractResult.emplace(std::move(contractResult));
    return result;
