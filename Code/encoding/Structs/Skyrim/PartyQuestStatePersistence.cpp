...
constexpr uint32_t kMaxQuestCount =
    static_cast<uint32_t>(PartyQuestResourcePolicy::MaxWireQuestEntries);
constexpr uint64_t kMaxJournalEntries =
    PartyQuestDurableResourcePolicy::MaxCanonicalJournalRecords;
constexpr uint32_t kMaxCollectionEntries =
    static_cast<uint32_t>(PartyQuestResourcePolicy::MaxSnapshotCollectionEntries);
...
PartyQuestPersistenceStatus ReadFile(const std::filesystem::path& acPath, std::vector<uint8_t>& aBytes)
{
    ...
    if (!file.is_open())
    {
        ...
        return ec || exists
            ? PartyQuestPersistenceStatus::IoError
            : PartyQuestPersistenceStatus::FileNotFound;
    }
...
PartyQuestPersistenceResult PartyQuestStatePersistence::Load(const std::filesystem::path& acPath)
{
    ...
    {
        primaryResult.Status = PartyQuestPersistenceStatus::InvalidData;
